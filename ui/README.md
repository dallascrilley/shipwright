...
- `list-shipwright-runs` / `get-shipwright-run` / `cancel-shipwright-run`: searchable/filterable history with opaque cursor paging and retention summary, detail, cancel. Search covers target owner/repo/number/title, summary, and run-id prefix only — never receipt tails or error bodies. Terminal history is retained up to 500 records; active runs and their lineage ancestors are preserved.
