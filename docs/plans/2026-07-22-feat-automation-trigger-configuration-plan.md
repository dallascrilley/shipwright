...
- **spine_owner:** self
- **Pre-flight:** from the Shipwright implementation worktree, run `git worktree list --porcelain`, `wt -C . list --full`, `git status --short`, and check `.agents-state/worktrees.json` if it exists; compare every active Shipwright worktree against the write surfaces below. This repository has no `.config/wt.toml`, so native Git remains the source of truth and Worktrunk provides the secondary status view. The available `~/.hub/scripts/worktree-posture.sh` is Hub-repository-specific and must not be used to create a false Shipwright claim.
