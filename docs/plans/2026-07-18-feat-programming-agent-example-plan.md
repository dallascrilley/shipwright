...
- [ ] U5 proof: deterministic suite, typecheck, and real Docker lifecycle pass; Kimi K3 model configuration and its OpenAI-compatible stream are verified, and the AgentOS/Pi sidecar frame deadline now follows the configured agent deadline. The one bounded local Pi acceptance run passed the former two-minute cutoff, but did not yield a terminal test receipt before its 300-second harness deadline; live GitHub proof still awaits an explicitly configured disposable issue and App installation.
...
- Observation: Kimi K3 accepts the configured `https://api.kimi.com/coding/v1` endpoint and streams standard OpenAI-compatible SSE, but rejects the OpenAI `developer` role with `400 Invalid request: tokenization failed`; Pi must send the system prompt as `system`. AgentOS 0.2.7 then applied its fixed 120-second frame deadline to the whole ACP `session/prompt` request, while Pi only returns that request after its complete tool loop. Evidence: bounded local Pi fixture runs, local Pi/SSE controls, and installed AgentOS/Pi source on 2026-07-19.
- Observation: after the explicit-sidecar patch, the bounded local Pi test remained active beyond the former 120-second frame cutoff. Its host process had ended by 300 seconds, the test's declared deadline, without a terminal receipt being returned to the controlling sidecar; the now-idle test-owned Docker sidecar was removed. Evidence: process and container inspection on 2026-07-19.
...
- Decision: configure Kimi's Pi catalog with `compat.supportsDeveloperRole: false`. Rationale: this is the smallest provider-specific override that matches Kimi's observed OpenAI-compatible request behavior without weakening model, tool, or publication policy. Date/Author: 2026-07-19 / Codex.
- Decision: apply a Bun-managed patch to `@rivet-dev/agentos-core` 0.2.7 that exposes the sidecar frame deadline on explicit sidecars, then create one explicit sidecar per programming-agent run with a one-minute cleanup buffer beyond the selected Pi deadline. Rationale: it fixes the confirmed timeout mismatch without changing the agent prompt, Kimi configuration, sandbox permissions, or global shared-sidecar behavior. Re-evaluate the patch when AgentOS upgrades. Date/Author: 2026-07-19 / Codex.
...
The AgentOS/Pi transport now creates an explicit sidecar whose protocol-frame deadline is the selected Pi deadline plus a one-minute cleanup buffer. The focused runner test and full deterministic suite prove that wiring and cleanup; `bun run typecheck` passed. One Kimi acceptance run exceeded the former fixed 120-second failure point, but its five-minute test harness expired before the agent returned a terminal result. The remaining local acceptance blocker is therefore Kimi/Pi completion latency or behavior, not the AgentOS two-minute sidecar frame deadline. No remote change was made, and the orphaned test sidecar was removed.
...
- 2026-07-19: Added the Kimi K3 system-role compatibility override after a real local Pi run reproduced Kimi's `developer`-role rejection; recorded the remaining AgentOS/Pi sidecar timeout that prevents local edit/test acceptance.
- 2026-07-19: Added a Bun-managed AgentOS sidecar-timeout patch and per-run explicit sidecar lifecycle to align the protocol frame deadline with the selected Pi timeout; scheduled one bounded local Kimi acceptance proof under the existing Code Plan quota with no recurring runtime or new monetary charge.
- 2026-07-19: Ran the single bounded Kimi local acceptance proof. It outlasted the former AgentOS 120-second cutoff but did not return before the test's 300-second deadline; removed the test-owned idle Docker sidecar and recorded model completion as the remaining local blocker.
