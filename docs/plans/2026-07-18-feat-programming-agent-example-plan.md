...
- [ ] U5 proof: deterministic suite, typecheck, and real Docker lifecycle pass; Kimi K3 model configuration and its OpenAI-compatible stream are verified, but the AgentOS/Pi sidecar currently times out waiting for a protocol frame during the real Pi edit/test. Live GitHub proof also awaits an explicitly configured disposable issue and App installation.
...
- Observation: Kimi K3 accepts the configured `https://api.kimi.com/coding/v1` endpoint and streams standard OpenAI-compatible SSE, but rejects the OpenAI `developer` role with `400 Invalid request: tokenization failed`; Pi must send the system prompt as `system`. After that compatibility override, the AgentOS/Pi sidecar timed out waiting for a protocol frame before the local fixture could be edited. Evidence: bounded local Pi fixture runs and low-token Kimi probes on 2026-07-19; no Kimi quota was consumed.
